...
from utils.data_sampler import DistInfiniteBatchSampler, EvalDistributedSampler, DistWeightedInfiniteBatchSampler
...
def build_everything(args: arg_util.Args):
    ...
    if not args.local_debug:
        ...
        num_classes, dataset_train, dataset_val = build_dataset(
            args.data_path, args.data_path_val, final_reso=args.data_load_reso, hflip=args.hflip,
        ...
        ld_train = DataLoader(
            ...
            batch_sampler=DistWeightedInfiniteBatchSampler(
                world_size=dist.get_world_size(),
                rank=dist.get_rank(),
                dataset_len=len(dataset_train),
                glb_batch_size=args.glb_batch_size,
                sample_classes=dataset_train.sample_classes,  # Pass from your dataset
                fill_last=True,
                shuffle=True,
                start_ep=start_ep,
                start_it=start_it,
    ...
    from trainer_refiner import VARTrainer
    ...
    # Load VAR trained for restoration
    var_ckpt = args.var_ckpt
    m, u = var_wo_ddp.load_state_dict(torch.load(var_ckpt, map_location='cpu')['trainer']['var_wo_ddp'],
                                      strict=False)
    print(f"Missing keys: {m}, Unexpected keys: {u}")

    for name, param in var_wo_ddp.named_parameters():
        if 'vae_refiner' not in name:
    ...
    def stats_str(name, m):
        total, trainable = param_stats(m)
        frozen = total - trainable
        pct = 100.0 * trainable / total if total > 0 else 0.0
        return (
            f'{name}: total={fmt_m(total)}, '
            f'trainable={fmt_m(trainable)}, '
            f'frozen={fmt_m(frozen)} ({pct:.1f}% trainable)'
        )

    print('[INIT][PARAMS] ' + stats_str('VAR', var_wo_ddp))
    print('[INIT][PARAMS] ' + stats_str('VAE', vae_local))
    print('[INIT][PARAMS] ' + stats_str('VAE.enc', vae_local.encoder))
    print('[INIT][PARAMS] ' + stats_str('VAE.dec', vae_local.decoder))
    print('[INIT][PARAMS] ' + stats_str('VAE.quant', vae_local.quantize))
    print()
...
def main_training():
    ...
    # TODO: Call wandb log after allreduce, currently only logs main process
    if dist.is_master():
        wandb.init(
            project=args.wandb_project if hasattr(args, "wandb_project") else "RestoreVAR",  # args doesnt have these
            entity=args.wandb_entity if hasattr(args, "wandb_entity") else None,
            config=args,  # log all arguments as configuration
            name=f"{args.exp_name}_{time.strftime('%Y%m%d_%H%M%S')}",
            save_code=True  # saves all *.py files in the current directory
        )
        # Save all python files to wandb (this uploads code snapshots)
        wandb.save("*.py")
    ...
    for ep in range(start_ep, args.ep):
        ...
        ### New, changed to save every epoch
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger,
    ...
    ### New, to get degraded image as well
    for it, (inp, degrad, label, restoration_ds, sample_idx) in me.log_every(start_it, iters_train,
                                                                                         ld_or_itrt,
                                                                                         30 if iters_train > 8000 else 50,
                                                                                         header):
        ...
        degrad = degrad.to(args.device, non_blocking=True)  # degraded image condition

        args.cur_it = f'{it + 1}/{iters_train}'

        wp_it = args.wp * iters_train
        min_tlr, max_tlr, min_twd, max_twd = lr_wd_annealing(args.sche, trainer.var_opt.optimizer, args.tlr, args.twd,
        ...
        grad_norm, scale_log2 = trainer.train_step(
            ...
            inp_B3HW=inp, degrad_B3HW=degrad, label_B=label, prog_si=prog_si,
            prog_wp_it=args.pgwp * iters_train,
        )  # Add degraded image as argument

        # Log to wandb
        if dist.is_master():
            wandb.log({
                "Global iter (g_it)": g_it,
                "tlr (actual lr)": max_tlr,
                "Epoch": ep
            })
