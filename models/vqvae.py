...
class VQVAE(nn.Module):
    ...
    def img_to_f(self, inp_img_no_grad: torch.Tensor, v_patch_nums: Optional[Sequence[Union[int, Tuple[int, int]]]] = None) -> List[torch.LongTensor]:    # return List[Bl]
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        B, C, H, W = f.shape
        return f
    
    def img_to_fs(self, inp_img_no_grad: torch.Tensor) -> torch.Tensor:    # return List[Bl]
        """
        Continuous conditioning
        """
        fs = self.encoder(inp_img_no_grad, return_lvls=True)
        # Filter fs to reduce token length
        indices = [1, 3, 5, -1]
        fs = [fs[i] for i in indices]
        interpol_shape = self.quantize.v_patch_nums[-1] ## 512 resolution, for 256 put 16
        lvls_resized = []
        for f in fs:
            f_resized = F.interpolate(f, size=(interpol_shape, interpol_shape), mode='bicubic')
            B, C, H, W = f_resized.shape
            lvls_resized.append(f_resized.view(B, C, -1).transpose(1, 2))
        return lvls_resized

    def idxBl_to_f(self, ms_idx_Bl: List[torch.Tensor], same_shape: bool, last_one=False) -> Union[List[torch.Tensor], torch.Tensor]:
        ...
        if last_one:
            return self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=True, last_one=True)
        else:
            return [f_hat for f_hat in
                    self.quantize.embed_to_fhat(ms_h_BChw, all_to_max_scale=True, last_one=False)]
