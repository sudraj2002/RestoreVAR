...
class VAR(nn.Module):
    def __init__(
            ...
            flash_if_available=True, fused_if_available=True, rope_base=10000.0, inference_mode=False
    ):
        super().__init__()
        # 0. hyperparameters
        ...
        if inference_mode:
            # For training, we first train without refiner
    ...
    def forward(self, x_BLCv_wo_first_l: torch.Tensor,
                cond_BLCv_wo_first_l: torch.Tensor) -> torch.Tensor:  # returns logits_BLV
        """
        ...
        with torch.cuda.amp.autocast(enabled=False):
            # SOS, class embedding, not very relevant for restoration
            label_B = torch.full((B,), fill_value=self.num_classes).to(
                x_BLCv_wo_first_l.device)  # Fill all label_B to 1000
            sos = cond_BD = self.class_emb(label_B)
            sos = sos.unsqueeze(1).expand(B, self.first_l, -1)

            # Project conditioning (last encoded latent)
            fs_proj4 = self.cond_proj4(cond_BLCv_wo_first_l[3].float())
            ...
            sos = sos + global_embed  # Add it to sos
            cond_BD = cond_BD + global_embed.squeeze(1)  # Add it to cond_BD

            if self.prog_si == 0:
                assert False, "Progressive training is not supported."
            else:
                # construct teacher-forcing input from clean image tokens
                x_BLC = torch.cat((sos, self.word_embed(x_BLCv_wo_first_l.float())), dim=1)
                # Get sequence for conditioning image
                cond_BLC = self.word_embed_cond(cond_BLC_proj.float())

            # Level embedding
            x_BLC += self.lvl_embed(self.lvl_1L[:, :ed].expand(B, -1))
            # Add pos embed to cond_BLC
            cond_BLC += self.lvl_embed_cond(self.lvl_1L_cond[:, :].expand(B, -1))

        # Attention mask for teacher forcing
        ...
        cond_BLC = cond_BLC.to(dtype=main_type)  # adjust dtype of condition
        cond_BD_or_gss = cond_BD_or_gss.to(dtype=main_type)
        attn_bias = attn_bias.to(dtype=main_type)

        AdaLNSelfAttn.forward
        for i, b in enumerate(self.blocks):
            x_BLC = b(x=x_BLC, cond_BD=cond_BD_or_gss, attn_bias=attn_bias, condition=cond_BLC,
                      freqs_cis=self.freqs_cis,
                      freqs_cis_cross=self.freqs_cis_cross)  # Pass condition for cross attention
    ...
    def autoregressive_infer_cfg(
        ...
        # Get continuous conditioning tokens
        cond_BLCv_wo_first_l = self.vae_proxy[0].img_to_fs(cond_img)
        ...
        label_B = torch.full((B,), fill_value=self.num_classes).to(
            cond_BLCv_wo_first_l[0].device)  # Fill all label_B to 1000

        # SOS
        sos = cond_BD = self.class_emb(label_B)

        # Project condition (final encoded latent)
        fs_proj4 = self.cond_proj4(cond_BLCv_wo_first_l[3].float())
        cond_BLC_proj = torch.cat([fs_proj4], dim=1)

        # Inject global embedding
        global_embed = self.global_embed_gain * self.global_embed(
            cond_BLC_proj.mean(dim=1, keepdim=True))  # Get global embedding
        cond_BD = cond_BD + global_embed.squeeze(1)  # Add it to cond_BD for AdaLN
        sos = sos.unsqueeze(1).expand(B, self.first_l, -1)
        sos = sos + global_embed

        # Level and condition embedding
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            # Get discrete next scale representation
        ...
        return self.vae_proxy[0].fhat_to_img(f_hat).add_(1).mul_(
            0.5), f_hat  # de-normalize, from [-1, 1] to [0, 1]

    def extra_repr(self):
        return f'drop_path_rate={self.drop_path_rate:g}'

    def debug_images(self, x_BLC, save_dir, step):
        ...
        f_hat = torch.zeros(B, self.Cvae, self.patch_nums[-1], self.patch_nums[-1]).to(x_BLC.device)

        # Iterate over each scale defined by self.patch_nums
        for si, pn in enumerate(self.patch_nums):
            # Extract logits for the current scale
            logits_scale = x_BLC[:, current_idx:current_idx + pn * pn, :]  # Shape: (B, pn * pn, V)
            current_idx += pn * pn
            idx_scale = logits_scale.argmax(dim=-1)

            # Convert sampled indices to embeddings
            h_scale = self.vae_quant_proxy[0].embedding(idx_scale)  # Shape: (B, pn, Cvae)
            h_scale = h_scale.transpose_(1, 2).reshape(B, self.Cvae, pn, pn)  # Shape: (B, Cvae, H, W)
            f_hat, _ = self.vae_quant_proxy[0].get_next_autoregressive_input(si, len(self.patch_nums),
                                                                             f_hat, h_scale)
