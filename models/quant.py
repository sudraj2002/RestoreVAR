...
class VectorQuantizer2(nn.Module):
    ...
    def cond_idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> torch.Tensor:
        next_scales = []
        B = gt_ms_idx_Bl[0].shape[0]
        C = self.Cvae
        H = W = self.v_patch_nums[-1]
        ...
        for si in range(SN):
            ...
            h_BChw = F.interpolate(self.embedding(gt_ms_idx_Bl[si]).transpose_(1, 2).view(B, C, pn, pn),
                                   size=(H, W), mode='bicubic')
            f_hat.add_(self.quant_resi[si / (SN - 1)](h_BChw))
            if si != SN - 1:
                next_scales.append(
                    F.interpolate(f_hat, size=(pn, pn), mode='area').view(B, C, -1).transpose(1, 2))
            else:
                next_scales.append(
                    f_hat.view(B, C, -1).transpose(1, 2))

        return torch.cat(next_scales, dim=1) if len(next_scales) else None  # cat BlCs to BLC, this should be float32

    def cont_interp(self, g) -> torch.Tensor:
        # next_scales = []
        B = g.shape[0]
        C = self.Cvae
        H = W = self.v_patch_nums[-1]  # 32x32
        SN = len(self.v_patch_nums)  # 10 scales
        f_rest = g.clone()
        f_hat = g.new_zeros(B, C, H, W, dtype=torch.float32)

        for si in range(SN):  # 10 scales
            pn = self.v_patch_nums[si]  # patch at scale
            if si != SN - 1:
                # next_scales.append(F.interpolate(f_rest, size=(pn, pn), mode='area').view(B, C, -1).transpose(1, 2))
                curr_scale = F.interpolate(f_rest, size=(pn, pn), mode='area').view(B, C, -1).transpose(1, 2)
            else:
                # next_scales.append(g.view(B, C, -1).transpose(1, 2))
                curr_scale = f_rest.view(B, C, -1).transpose(1, 2)

            h_BChw = F.interpolate(curr_scale.transpose_(1, 2).view(B, C, pn, pn),
                                   size=(H, W), mode='bicubic')
            # h_BChw = self.quant_resi[si / (SN - 1)](h_BChw)
            f_hat = f_hat + h_BChw
            f_rest = f_rest - h_BChw
        print(torch.abs(f_hat - g).mean())
        return f_hat  # cat BlCs to BLC, this should be float32
    
    def get_continuous_cond(self, g) -> torch.Tensor: 
        next_scales = []
        B = g.shape[0]
        C = self.Cvae
        H = W = self.v_patch_nums[-1]
        ...
        for si in range(SN):
            pn = self.v_patch_nums[si]
            if si != SN - 1:
                next_scales.append(F.interpolate(g, size=(pn, pn), mode='area').view(B, C, -1).transpose(1, 2))
            else:
                next_scales.append(g.view(B, C, -1).transpose(1, 2))
